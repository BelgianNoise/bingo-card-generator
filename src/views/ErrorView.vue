<script setup lang="ts">
  import IconHome from '@/components/icons/IconHome.vue';
  import { ref } from 'vue';
  import { useRoute, useRouter } from 'vue-router';

  const route = useRoute()
  const router = useRouter()
  const code = ref(route.query.code)
</script>

<template>
  <div>
    <h1>Woops, something went wrong!</h1>
    <svg
      viewBox="0 0 64 64"
      xmlns="http://www.w3.org/2000/svg"
      xmlns:xlink="http://www.w3.org/1999/xlink"
      aria-hidden="true" role="img"
      class="iconify iconify--emojione"
      preserveAspectRatio="xMidYMid meet"
      fill="#000000"
    >
      <path d="M2 32c0 16.6 13.4 30 30 30s30-13.4 30-30S48.6 2 32 2S2 15.4 2 32z" fill="#ffdd67"> </path>
      <path d="M47.7 37.6c0 8.6 11.7 8.6 11.7 0c0-6.3-5.8-12.4-5.8-12.4s-5.9 6.1-5.9 12.4" fill="#65b1ef"> </path>
      <g fill="#664e27">
        <circle cx="38.5" cy="35" r="5"> </circle>
        <circle cx="15.5" cy="35" r="5"> </circle>
      </g>
      <g fill="#917524">
        <path d="M20.6 19.9c-3.2 2.7-7.5 3.9-11.7 3.1c-.6-.1-1.1 2-.4 2.2c4.8.9 9.8-.5 13.5-3.6c.5-.5-1-2.1-1.4-1.7"> </path>
        <path d="M45.1 22.9c-4.2.7-8.5-.4-11.7-3.1c-.4-.4-2 1.2-1.4 1.7c3.7 3.2 8.7 4.5 13.5 3.6c.7-.2.2-2.3-.4-2.2"> </path>
      </g>
      <path d="M38.1 47.3c-5.8-1.5-12-.4-16.9 3c-1.2.9 1.1 4 2.3 3.2c3.2-2.3 8.4-3.8 13.7-2.4c1.3.3 2.4-3.3.9-3.8" fill="#664e27"> </path>
    </svg>
    <p>code: {{ code }}</p>
    <button class="secondary" @click="router.push('/')">
      Go back home
      <IconHome class="icon" color="var(--color-primary)" />
    </button>
  </div>
</template>

<style scoped>
  svg {
    max-height: 60%;
    max-width: 80%;
  }
  div {
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
    gap: var(--gap-normal);
    text-align: center;
    flex: 1;
    max-height: 70svh;
  }
</style>